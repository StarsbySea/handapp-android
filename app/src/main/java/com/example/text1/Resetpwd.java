...
//        layout.setOrientation(RelativeLayout.VERTICAL).
        ...
        mPwdCheck = findViewById(R.id.resetpwd_edit_pwd_check);

        mSureButton = findViewById(R.id.resetpwd_btn_sure);
        mCancelButton = findViewById(R.id.resetpwd_btn_cancel);
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确,继续后续操作
                if(userPwd_new.equals(userPwdCheck)==false){           //两次密码输入不一样
                    ...
                    if (flag == false) {
                        ...
                        Intent intent_Register_to_Login = new Intent(com.example.text1.Resetpwd.this,Login.class) ;    //切换User Activity至Login Activity
